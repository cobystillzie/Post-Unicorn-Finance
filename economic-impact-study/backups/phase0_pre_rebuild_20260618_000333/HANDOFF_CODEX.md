...
- `data/pilot_companies.csv` — **31 companies (15 post-unicorn, 16 VC-backed), all adversarially verified**,
  matched by sector + country, every cell source-cited.
- `analysis/pilot_first_cuts.md` — coverage gap map + value-sharing contrast (15/15 vs 1/16) + an
  illustrative per-dollar table.
- `visuals/study_architecture.html` — one-page diagram.
- `build_phase0.py` — regenerates `02/03/04`, the CSV, and first-cuts from the verified research JSON
  (path is hard-coded near the top; the JSON is the saved workflow result).
