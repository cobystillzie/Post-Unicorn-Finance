# AGENTS.md — Post-Unicorn Finance (operational source of truth)
...
## Data model — CSV is authority, SQLite is a cache

- `data/evidence/*.csv` are the source of truth (git-tracked). `data/atlas.sqlite` is a rebuildable cache
  (gitignored). Build/refresh with `import_csvs(DEFAULT_DB, reset=False)` (preserves the `source_pages` fetch
  cache) or `reset=True` (full wipe + re-fetch).
- Key tables: `industry_entities` (the atlas), `entity_intake_queue` (leads), `entity_claims`
  (one+ source-backed claim per entity), `source_registry`, `source_pages` (fetched homepage text, cache only).
